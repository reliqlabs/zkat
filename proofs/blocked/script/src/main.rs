...
const ELF: Elf = include_elf!("blocked-program");

#[derive(Parser)]
#[command(name = "blocked", about = "Prove that a DID blocked another DID")]
struct Args {
...
fn main() -> Result<()> {
    let args = Args::parse();
    let proof = prepare_record_proof(&args.did, "app.bsky.graph.block", &args.rkey, args.pds.as_deref())?;
    ...
    match args.mode.as_str() {
        "execute" => {
            ...
            println!("DID {} blocked {}", r.did, r.subject_did);
        ...
        "prove" => {
            ...
            println!("Proof generated. DID {} blocked {}", r.did, r.subject_did);
