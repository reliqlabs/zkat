...
use reposted_lib::{ProofInput, ProofOutput};

const ELF: Elf = include_elf!("reposted-program");

#[derive(Parser)]
#[command(name = "reposted", about = "Prove that a DID reposted a specific post")]
struct Args {
...
fn main() -> Result<()> {
    let args = Args::parse();
    let proof = prepare_record_proof(&args.did, "app.bsky.feed.repost", &args.rkey, args.pds.as_deref())?;
    ...
    match args.mode.as_str() {
        "execute" => {
            ...
            println!("DID {} reposted {}", r.did, r.subject_uri);
        ...
        "prove" => {
            ...
            println!("Proof generated. DID {} reposted {}", r.did, r.subject_uri);
