...
const ELF: Elf = include_elf!("liked-program");

#[derive(Parser)]
#[command(name = "liked", about = "Prove that a DID liked a specific post")]
struct Args {
...
fn main() -> Result<()> {
    let args = Args::parse();
    let proof = prepare_record_proof(&args.did, "app.bsky.feed.like", &args.rkey, args.pds.as_deref())?;
    ...
    match args.mode.as_str() {
        "execute" => {
            ...
            println!("DID {} liked {}", r.did, r.subject_uri);
        ...
        "prove" => {
            ...
            println!("Proof generated. DID {} liked {}", r.did, r.subject_uri);
