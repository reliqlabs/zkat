...
const ELF: Elf = include_elf!("list-member-program");

#[derive(Parser)]
#[command(name = "list-member", about = "Prove that a DID added another DID to a specific list")]
struct Args {
...
fn main() -> Result<()> {
    let args = Args::parse();
    let proof = prepare_record_proof(&args.did, "app.bsky.graph.listitem", &args.rkey, args.pds.as_deref())?;
    ...
    match args.mode.as_str() {
        "execute" => {
            ...
            println!("DID {} added {} to list {}", r.did, r.subject_did, r.list_uri);
        ...
        "prove" => {
            ...
            println!("Proof generated. DID {} added {} to list {}", r.did, r.subject_did, r.list_uri);
